...
export async function GET(_req: Request, context: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    const [{ data: order, error: orderError }, { data: events }, { data: files }, { data: history }, { data: submissionEvents }] = await Promise.all([
    ...
    return NextResponse.json({ order, events: events ?? [], files: files ?? [], history: history ?? [], submissionEvents: submissionEvents ?? [] });
  } catch (error) {
    console.error(error);
